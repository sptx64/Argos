...
def logging(secret1, secret2) :
    ...
    if "password" not in st.session_state :
        ...
        with col2.form("logging"):
            password1 = st.text_input("Insert access password1", secret1, type="password").replace("/","").replace(":","").replace("'","")
            password2 = st.text_input("Insert access password2", secret2, type="password").replace("/","").replace(":","").replace("'","")
