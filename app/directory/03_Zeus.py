...
def div(df, close, indicator, indicator_filter) :
    ...
    df_bottom = df[df['bot'].values == True].reset_index(drop=True)
    ...
    df_top = df[df['top'].values == True].reset_index(drop=True)
...
if rsi5014 :
    subplot+=1
    lowtf,htf=6,38
    data[f"RSIoscillator"] = ((RSI(data, lowtf)-50)*data["Volume"].rolling(lowtf).mean()+(RSI(data, htf)-50)*data["Volume"].rolling(htf).mean())/(data["Volume"].rolling(lowtf).mean()+data["Volume"].rolling(htf).mean())
    # data[f"RSIoscillator"] = RSI(data, 14)-RSI(data, 100)

    data[f"RSIoscillator"]=data[f"RSIoscillator"].rolling(3).mean()
    data["RSIOCalls"] = "Neural"
...
if WT :
    ...
    up_wick = [h-c if c>o else h-o for h,o,c,l in zip(data["High"].values, data["Open"].values, data["Close"].values, data["Low"].values)]
    low_wick = [o-l if c>o else c-l for h,o,c,l in zip(data["High"].values, data["Open"].values, data["Close"].values, data["Low"].values)]
    data["wick"] = [ l-u for l,u in zip(low_wick, up_wick)]
    data["wick2"]=(data["Close"]-data["Low"]) - (data["High"]-data["Close"])
    data["wick"]= (data["wick"]+data["wick2"])/2
    # data["wick"] = data["wick"].ewm(span=20).sum()
    data["wick"] = data["wick"].rolling(20).sum()#(span=20, adjust=False).mean()
...
if grid_search:
    marker_opacity = None
    if not gs_multiwindow :
        if grid_type == "Volume cum" :
            data_cut = data[data[grid_type] > data[grid_type].quantile(0.9)]
        else :
            data_cut = data[data[grid_type] <= 0]
        fig.add_trace(go.Scatter(x=data_cut["Date"].values, y=data_cut["High"].values, mode="markers", marker_color="violet", name=grid_type), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
        fig.add_trace(go.Scatter(x=data_cut["Date"].values, y=data_cut["Low"].values, mode="markers", marker_color="violet", name=grid_type), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    else :
        data.loc[(data[grid_type + "_perc"].values<0), grid_type + "_perc"] = 0
        data.loc[(data[grid_type + "_perc"].values>1), grid_type + "_perc"] = 1
        data.loc[(data[grid_type + "_perc"].isna()), grid_type + "_perc"] = 0

        data[grid_type + "_perc"] = data[grid_type + "_perc"]**7
        fig.add_trace(go.Scatter(x=data["Date"].values, y=data["High"].values, mode="markers", marker_color="violet", name=grid_type, marker_opacity=data[grid_type + "_perc"]), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
...
for cn in ma_cns :
    ...
    fig.add_trace(go.Scatter(x=data["Date"].values, y=data[cn].values, name=cn, mode="lines", line_width=1, line_color=dict_ma_colors[ma]), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
...
if DOT :
    fig.add_trace(go.Scatter(x=data[data['sentiment']=='bearish']["Date"], y=data[data['sentiment']=='bearish']["dot_y"], name="bear-dot", mode="markers", marker_color='red', marker_size=5), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    fig.add_trace(go.Scatter(x=data[data['sentiment']=='bullish']["Date"], y=data[data['sentiment']=='bullish']["dot_y"], name="bull-dot", mode="markers", marker_color='green', marker_size=5), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    fig.add_trace(go.Scatter(x=data["Date"], y=data['trendline'], name="20²d ema", mode='lines', line_width=1, line_color='yellow'), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
...
if WT :
    fig.add_trace(go.Scatter(x=data_bull_tick["Date"].values, y=data_bull_tick["Low"].values, mode='markers', marker_color='GreenYellow', marker_symbol="triangle-up", showlegend=False), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    fig.add_trace(go.Scatter(x=data_bear_tick["Date"].values, y=data_bear_tick["High"].values, mode='markers', marker_color='IndianRed', marker_symbol="triangle-down", showlegend=False), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    # fig.add_trace(go.Scatter(x=data_bull_tick["Date"].values, y=data_bull_tick["wick"].values, mode='markers', marker_color="green", showlegend=False), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    # fig.add_trace(go.Scatter(x=data_bear_tick["Date"].values, y=data_bear_tick["wick"].values, mode='markers', marker_color="crimson", showlegend=False), col=None if subplot==0 else 1, row=None if subplot==0 else 1)

if rsi5014 :
    bull_rsi5014 = data[data["RSIOCalls"].values=="Bullish"].reset_index(drop=True)
    bear_rsi5014 = data[data["RSIOCalls"].values=="Bearish"].reset_index(drop=True)

    fig.add_trace(go.Scatter(x=bull_rsi5014["Date"].values, y=bull_rsi5014["Low"].values, mode='markers', marker_color='GreenYellow', marker_symbol="triangle-up", showlegend=False), col=None if subplot==0 else 1, row=None if subplot==0 else 1)
    fig.add_trace(go.Scatter(x=bear_rsi5014["Date"].values, y=bear_rsi5014["High"].values, mode='markers', marker_color='Crimson', marker_symbol="triangle-down", showlegend=False), col=None if subplot==0 else 1, row=None if subplot==0 else 1)

if sqz :
    data["upper_BB"] = upperBB
    data["lower_BB"] = lowerBB

    sqz_on = data[data["Squeeze"]=="sqz on"].reset_index()
...
if rsi5014 :
    data, data_bottom, data_top = div(data, "Close", "RSIoscillator", None)
    ...
    for i in range(len(data_bottom)):
        ...
        if row['bullish_div'] == True :
            x = [row['Date'], prev_row['Date']]
            y = [row["RSIoscillator"], prev_row["RSIoscillator"]]
            fig.add_trace(go.Scatter(x=x, y=y, mode='markers+lines+text', line_color='limegreen', line_width=1, line_dash="dot", text=["BuD", "BuD"], textposition="bottom center", showlegend=False), col=1, row=subplot_row)
    ...
    fig.add_trace(go.Scatter(x=data["Date"].values, y=data[f"RSIoscillator"].values, name="RSIoscillator5014", mode="lines", line_width=1), col=1, row=subplot_row)
    fig.add_hline(y=0, line_width=1, line_color="black", row=subplot_row)
    fig.add_hline(y=20, line_width=1, line_color="black", row=subplot_row)
    fig.add_hline(y=-20, line_width=1, line_color="black", row=subplot_row)
