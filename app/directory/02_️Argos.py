...
go = c1.button(":material/search: Let's go!", type="primary")
compile_results = c2.toggle("Compile result graph")
if compile_results :
    comp_res_argos = []
...
if go :
    ...
    for symbol in tickers :
        ...
        try :
            data = pd.read_parquet(os.path.join(path,f"{symbol}.parquet"))
        except :
            continue
        ...
        if len(data) > 0 :
            ...
            if (len(df_check) == count) & (len(df_check) > 0) :
                if compile_results :
                    ...
                    dataf["Symbol"] = symbol
                    dataf["Volume_ratio"] = dataf["Volume"]/dataf["Volume"].quantile(0.9)
                    comp_res_argos.append(dataf)

                @st.fragment
                def result_argos(symbol, market) :
                    c1,c2 = st.columns([1,5])
                    if market == 'crypto' :
                        link = f"[{symbol}(Binance)](https://www.tradingview.com/chart/?symbol=BINANCE:{symbol})"
                        c1.write(link + ' : ' + df_check.to_string(index=False).replace('ta_ref', '').replace('result', ''))
                    else :
                        ...
                        c1.write(links[0] + '/' + links[1] + ' : ' + df_check.to_string(index=False).replace('ta_ref', '').replace('result', ''))

                    if c2.button(f"Show chart {symbol}") :
                        dataframe = pd.read_parquet(os.path.join(path, f"{symbol}.parquet"))
                ...
                result_argos(symbol, market)


    if compile_results :
        comp_res_argos = pd.concat(comp_res_argos)
        if len(comp_res_argos) > 0 :
