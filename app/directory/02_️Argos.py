...
with c1.expander("Oscillator", expanded=True) :
    cl1, cl2 = st.columns(2)
    ab_rsi, un_rsi = cl1.checkbox('RSI : above', help="find RSI setup in a particular range"), cl2.checkbox('RSI : under', help="find RSI setup in a particular range")
    ab_rsi_number = cl1.slider('above rsi', 0, 100, 90, 10, help="Looking for all asset with RSI above this value") if ab_rsi else None
    un_rsi_number = cl2.slider('under rsi', 0, 100, 10, 10, help="Looking for all asset with RSI under this value") if un_rsi else None

    cl1, cl2 = st.columns(2)
    ab_k, un_k = cl1.checkbox('%K : above', help="find %K setup in a particular range"), cl2.checkbox('%K : under', help="find %K setup in a particular range")
    ab_k_number = cl1.slider('above %K', 0, 100, 90, 10, help="Looking for all asset with %K above this value") if ab_k else None
    un_k_number = cl2.slider('under %K', 0, 100, 10, 10, help="Looking for all asset with %K under this value") if un_k else None

    cl1, cl2 = st.columns(2)
    ao_breakout_up = cl1.checkbox("AO : break up", help="looking for an awesome oscillator breaking above 0 line")
    ao_breakout_down = cl2.checkbox("AO : break down", help="looking for an awesome oscillator breaking under 0 line")
        ...
        for sm in submarkets :
            ...
            for symbol in tickers :
                ...
                if len(data) > 20 :
                    ...
                    if ab_k :
                        count+=1
                        ab_k_result = ta.check_if_above_k(data, ab_k_number)
                        df_check.loc[df_check['ta_ref'] == 'above_k', 'result'] = str(ab_k_result)

                    if un_k :
                        count+=1
                        un_k_result = ta.check_if_under_k(data, un_k_number)
                        df_check.loc[df_check['ta_ref'] == 'under_k', 'result'] = str(un_k_result)

                    if ao_breakout_up or ao_breakout_down :
                        aos = ta.ao(data).to_list()
                    if ao_breakout_up :
                        count+=1
                        ao_up = ((aos[-1]>0) & (aos[-2]<0)) | ((aos[-2]>0) & (aos[-3]<0)) | ((aos[-3]>0) & (aos[-4]<0))
                        df_check.loc[df_check['ta_ref'] == 'ao_breakout_up', 'result'] = str(ao_up)

                    if ao_breakout_down :
                        count+=1
                        ao_down = ((aos[-1]<0) & (aos[-2]>0)) | ((aos[-2]<0) & (aos[-3]>0)) | ((aos[-3]<0) & (aos[-4]>0))
                        df_check.loc[df_check['ta_ref'] == 'ao_breakout_down', 'result'] = str(ao_down)
                    ...
                    if (len(df_check) == count) & (len(df_check) > 0) :
                        ...
                        len_graph = 100 if len(data)>100 else len(data)

                        close = data.tail(len_graph)["Close"].to_numpy()

                        metric_col[v_col].metric(symbol, round(close[-1],3), delta=round((close[-1]-close[-len_graph])/close[-len_graph]*100,2), chart_data=close, chart_type="line", border=True )
                        ...
                        @st.fragment
                        def result_argos(symbol, market, sm, weekly, text_str, value_button) :
                            c1,c2 = st.columns([1,5])
                            if market == 'crypto' :
                            ...
                            if c2.button(f"Show chart {symbol}", key=str(value_button)) :
                        ...
                        result_argos(symbol, market, sm, weekly, text_str, value_button)
